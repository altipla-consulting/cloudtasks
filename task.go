...
type Task struct {
	...
	scheduleTime time.Time     // timestamp of the task execution
	delay        time.Duration // delay in seconds before the task is executed
...
func (task *Task) LogValue() slog.Value {
	...
	if !task.scheduleTime.IsZero() {
		attrs = append(attrs, slog.Time("scheduleTime", task.scheduleTime))
	}
	if task.delay > 0 {
		attrs = append(attrs, slog.Duration("delay", task.delay))
	}
...
func WithDelay(seconds int32) TaskOption {
	return func(task *Task) {
		task.delay = time.Duration(seconds) * time.Second
	}
}
