...
type WorkflowsOption[TPayload any] func(state *runState[TPayload])

// WithName configures a custom name for the workflow. By default it will be autogenerated. A custom name could be problematic
// with tombstones (workflow names that can't be repeated) and concurrency controls, so assign it with care and read
func WithName[TPayload any](name string) WorkflowsOption[TPayload] {
	return func(state *runState[TPayload]) {
		state.ID = name
	}
}

func (w *Workflow[TPayload]) Start(ctx context.Context, queue cloudtasks.Queue, payload TPayload, opts ...WorkflowsOption[TPayload]) error {
	state := runState[TPayload]{
		ID:      ksuid.New().String(),
		Payload: payload,
	}
